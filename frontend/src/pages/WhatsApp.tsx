...
export function WhatsApp() {
  ...
  // No subscribe() here — AppHeader holds the one event stream for the whole
  // session. The read is still worth doing: the header only fetches once, on
  // mount, so arriving here after a stream that gave up would otherwise show a
  // stale status on the very page you came to fix it from.
  useEffect(() => {
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 pb-24 pt-10">
      ...
      <section className="mt-10 border-y border-rule py-8">
        ...
        {/* Without this the page would keep insisting "Awaiting scan" long after
            the stream stopped feeding it — the worst place to be silently wrong. */}
        {streamStale && (
          <div className="mt-5 border-l-2 border-amber-soft bg-amber-soft-bg/60 px-4 py-2 text-[12px] text-ink-soft">
            Live updates disconnected. This status was last confirmed on load —
