...
`frontend/scripts/test-dates.ts` is the same idea on the frontend, and is what `frontend`'s `npm run verify` runs: 29 assertions over `formatFriendly` / `formatCountdown` against a pinned `now`, covering calendar-day boundaries, the year rollover, and every rung of the countdown ladder. Run it after touching either helper. It exists because that logic already shipped a real bug — a single-unit countdown read "in 1 day" directly beneath a date two days out — and only a pinned `now` can prove the two helpers agree.
